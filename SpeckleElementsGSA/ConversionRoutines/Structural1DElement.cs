...
{
    [GSAObject("EL.3", new string[] { "NODE.2" }, "elements", true, false, new Type[] { typeof(GSANode) }, new Type[] { typeof(GSA1DProperty) })]
    ...
    public static partial class Conversions
    {
        ...
        public static bool ToNative(this Structural1DElement load)
        {
            if (Conversions.GSATargetLayer == GSATargetLayer.Analysis)
                new GSA1DElement() { Value = load }.SetGWACommand(GSA);
            else if (Conversions.GSATargetLayer == GSATargetLayer.Design)
                new GSA1DMember() { Value = load }.SetGWACommand(GSA);
